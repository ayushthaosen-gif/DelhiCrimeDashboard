...
| Traffic signals, pedestrian crossings, hospitals, footway/sidewalk coverage (interactive map only) | OpenStreetMap contributors | [OpenStreetMap data and licence](https://www.openstreetmap.org/copyright); [exact queries and processing notes](data/source/README.md) | Snapshot 2026-08-06, `scripts/fetch_osm_infra_snapshots.js` + `scripts/build_infra_extras.js`; footway coverage is a per-district length/density figure, not shipped as line geometry | ODbL; mapped inventory, not an official completeness register. |
