...
function initMap() {
  ...
  ACCIDENT_ZONES_MAPPED.forEach(z => {
    ...
    zoneParts.push('<path class="zone-marker" style="opacity:' + opacity + '" d="M' + x + ',' + (y - size) + ' L' + (x + size * 0.87) + ',' + (y + size * 0.67) + ' L' + (x - size * 0.87) + ',' + (y + size * 0.67) + ' Z" data-tt-title="' + esc(z.name + ' (' + z.road + ')') + '" data-tt-body="' + esc(z.district + ' · ' + z.fatal + ' fatal, ' + z.total + ' total crashes, 2023') + '"></path>');
  });
...
function renderMap() {
  ...
  document.querySelectorAll('#map path.district-hatch').forEach(path => {
    ...
    if (!d) return;
    const v = metricValue(d, m);
    const showHatch = (v == null) || (showLights && !d.surveyPoints);
