...
def _download_manifest(paths, year):
    manifest_path = paths.manifests / f"download_manifest_{year}.json"
    manifest = _json(manifest_path, {"year": year, "downloads": []})
    changed = False
    for item in manifest.get("downloads", []):
        saved = Path(item["local_path"])
        if saved.exists():
            continue
        rebased = paths.raw / item["source_id"] / saved.name
        if rebased.exists():
            item["local_path"] = str(rebased)
            changed = True
    if changed:
...
def _download_osm(source, client, paths) -> list[DownloadRecord]:
    ...
    boundary_path = PACKAGE_ROOT.parents[1] / "data" / "dashboard_boundaries_simplified.geojson"
...
def download(year=2025, source_id=None, offline=False, root=None):
    ...
    if offline:
        return _download_manifest(paths, year)
    client = _client(settings)
    records = []
    errors = []
    try:
...
def extract(year=2025, source_id=None, offline=False, root=None):
    ...
    downloads = _download_manifest(paths, year)
    extracted = []
    review = []
